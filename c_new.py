...
def is_corr_broken(corr_mat): # pass last values of the dataframe here
	...
	for item in corr_mat:
		count = count + 1
		if abs(item) > abs(last_corr_reading[count]) + abs(last_corr_std[count]) or abs(item) < abs(last_corr_reading[count]) - abs(last_corr_std[count]):
...
def reset_correlation(corr_mat_dataset):
	...
	last_corr_reading = corr_mat_dataset.to_numpy()[len(corr_mat.to_numpy())-1]
...
while i < 100:
	...
	if corr_saved_counter == sample_size or corr_hit_counter >= reset_calibration_step:
		corr_mat_dataset = read_csv('corr_data.csv')
		index = 1-i
		corr_mat_dataset = corr_mat_dataset.iloc[index]
		reset_correlation(corr_mat_dataset)


	broken_corr_mat
	if i>= sample_size*2:
		last_corr_mat = read_csv('corr_data.csv')[-1]
	...
	ceiling_hits_string = ""
	floor_hits_string = ""
	arima_arima_action_taken_string = ""
